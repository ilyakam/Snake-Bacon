...
function getPosition(input, tick) {
  var actions =
    input.left.map(function() { return headLeft }).merge(
    input.right.map(function() { return headRight })).merge(
    input.up.map(function() { return headUp })).merge(
    input.down.map(function() { return headDown }))
